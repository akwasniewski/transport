use crate::{
    algo::{algo_result::AlgoResult, alt::landmarks::potential, utils::QueueItem},
    graph::Graph,
};
...
pub fn alt(
    ...
    que.push(QueueItem::with_priority(
        from,
        OrderedFloat(0.0) + potential(from, to, landmarks),
        OrderedFloat(0.0),
    ));
    ...
    while !que.is_empty() {
        ...
        for c in &graph.vertices[cur.vertex].connections {
            ...
            if alt_cost < dist[*c.0] {
                que.push(QueueItem {
                    vertex: *(c.0),
                    priority: alt_cost + potential(*c.0, to, landmarks),
                    distance: alt_cost,
                });
                dist[*c.0] = alt_cost;
            }
